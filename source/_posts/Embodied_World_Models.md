...
## [LARY: A Latent Action Representation Yielding Benchmark for Generalizable Vision-to-Action Alignment](https://arxiv.org/pdf/2604.11689v1)
在 VLA 模型中，显式机器人动作数据稀缺，而互联网上存在大量人类视频和机器人视频。如果能从这些视频中抽取出与具体机器人形态无关的 **latent action representation**，就有可能缓解机器人动作数据不足的问题。但在此之前，领域内缺少一个系统 benchmark 来评估这些 latent action 是否真的能表示动作、是否能支持机器人控制。

已有 latent action model，例如 LAPA、Moto、LAPO 等，通常会把视频帧之间的视觉变化编码成离散或连续的 latent action tokens。这类方法的目标是把“视觉变化”转化成某种可迁移的动作表征，从而连接人类视频、机器人视频和 VLA 控制。但已有评估往往依赖下游策略表现或聚类可视化，难以把 latent action 本身的质量与具体 policy、训练环境、任务数据解耦。因此，一个模型在下游任务上表现好，并不一定说明它的 latent action 表征本身更好。LARY 正是为了解决这个评估缺口。

LARYBench 的核心贡献是构建了一个统一评测框架，把 latent action representation 的能力拆成两个层面：第一是 high-level semantic action，即“what to do”，评估模型是否能识别动作语义，例如抓取、推、打开、关闭等；第二是 low-level robotic control，即“how to do”，评估 latent action 是否能回归真实机器人控制信号，例如末端执行器位移、旋转和夹爪状态。通过这种设计，LARY 不再只看最终任务成功率，而是直接探测 representation 是否同时包含语义动作信息和物理控制信息。

在数据规模上，LARYBench 构建了一个非常大的跨实体、跨视角、跨环境数据集。论文和项目页给出的数据包括超过 100 万个视频片段、约 1000 小时视频、151 个动作类别，并包含 620K 图像对 和 595K 运动轨迹，覆盖人类和机器人、第一视角和第三视角、真实和仿真环境，以及多种机器人 embodiment。项目页还说明其覆盖 11 种 embodiment。

方法上，LARYBench 提供了三个基本流程。首先是 `get_latent_action`，即从视频或图像对中提取 latent action representation；其次是 `classification`，用线性或轻量 probe 测试 latent action 是否能进行动作语义分类；最后是 `regression`，测试 latent action 是否能解码出低层机器人动作。这个流程的好处是，不管一个模型是 LAM、通用视觉 encoder，还是视频生成 encoder，只要能输出某种 latent representation，就可以放进同一套评测协议中比较。Hugging Face 数据集页和 GitHub 都明确列出了这三个评测 pipeline。

论文系统比较了四类表示范式。第一类是专门为具身任务设计的 Embodied Latent Action Models，例如 LAPA、UniVLA、villa-X；第二类是 General Semantic Encoders，例如 DINOv3 和 V-JEPA 2；第三类是 Generative Pixel Encoders，例如 Wan2.2 VAE 和 FLUX.2-dev VAE；第四类是作者构造的 General Latent Action Models，即把 LAPA 框架中的视觉 encoder 替换为 DINO、SigLIP、MAGVIT2 等预训练 backbone，以分析通用视觉先验对 latent action learning 的帮助。

实验结论非常有意思。第一，**通用视觉基础模型反而比许多专门的 embodied latent action model 更强**。例如 DINOv3、V-JEPA 2 这类没有显式动作监督的大视觉模型，在动作语义分类和机器人动作回归上都表现出很强的 action-relevant representation。论文认为，这说明大规模视觉预训练本身已经编码了大量与动作相关的空间、时间和物体变化信息。

第二，论文发现 **latent-level visual representation 比 pixel-level reconstruction 更适合 vision-to-action alignment**。也就是说，机器人控制并不一定需要模型精确重建像素级未来图像，更重要的是捕捉语义层、结构层和动作相关的视觉变化。这个结论对当前 world model / VLA 研究很有启发：如果目标是控制，盲目追求高清像素重建未必是最优路径，能否形成更抽象、更动作相关的 latent space 可能更关键。

论文还通过消融实验讨论了什么样的 latent action model 更有效。作者在 LAPA 框架下替换不同视觉 encoder，并分析 codebook size、sequence length、latent dimension 等因素。结果显示，自监督视觉 encoder，例如 DINOv3，往往比纯重建式或视觉语言对比模型更能构建有效 latent action；同时，量化瓶颈需要适度设计，codebook 太小会限制表示能力，太大又可能导致 utilization 下降；sequence length 过短则容易造成 codebook collapse。

从研究意义看，LARY 的价值不在于提出一个新的 policy，而在于提供了一个评估 latent action representation 的“标尺”。它把 latent action 的质量从下游策略训练中独立出来，让研究者可以直接回答：某个表示是否真的包含动作语义？是否能映射到真实机器人控制？是否能跨人类、机器人、视角和环境泛化？这对后续利用大规模人类视频训练 VLA、构建 action-conditioned world model、研究 ontology-independent action space 都很重要。总体来看，LARY 给出的核心启示是：未来 VLA 不一定只能依赖稀缺的机器人动作标签，也不一定必须从零学习动作空间。大规模通用视觉模型中已经蕴含了相当多的动作知识，而更好的方向可能是把这些视觉先验转化为稳定、可迁移、能支持低层控制的 latent action representation。简言之，LARY 证明了“视觉表征本身可能已经很懂动作”，而 benchmark 的作用就是把这种能力用标准化方式测出来。
...
### 与FFDC的区别
FFDC 不是每个低层 action step 都检测一次，也不是只在整个 chunk 结束后检测一次，而是在执行 chunk 过程中，按视觉 token 对齐的检查点进行检测。也就是大致每隔r 个 action step 检测一次。
Cosmos Policy 默认不是按 action-to-video ratio 预测多帧未来图像，而是每个 action chunk 预测一个未来状态/value。

### 推理过程中真实截图
当前截图是每执行一步 action 都获取一帧，不是等整个 action chunk 执行完才获取一帧。

  具体顺序是：

  1. 前 10 步 NUM_STEPS_WAIT 只执行 dummy action，不保存截图。
  2. 之后每次 while 循环开始时：
      - 如果启用 wrist image，也 append wrist 图像
  3. 如果 action_queue 为空，才用这帧当前 observation 去 query model，得到一个 action chunk。
  - 当前帧获取/保存： cosmos_policy/experiments/robot/libero/run_libero_eval.py:455
  - action chunk 只在队列空时重新推理： cosmos_policy/experiments/robot/libero/run_libero_eval.py:466
  - 每次只取一个 action 执行： cosmos_policy/experiments/robot/libero/run_libero_eval.py:671
  - 执行环境一步： cosmos_policy/experiments/robot/libero/run_libero_eval.py:680
  - 当前截图获取频率：每循环一步获取一帧，也就是每执行一个 action 前获取当前帧。
  - future image prediction 保存频率：每个 action chunk 保存一次，因为它只在重新 query model 时 append 到 future_image_predictions_list。

换句话说，假设 chunk_size = num_open_loop_steps = 16，那么第一个 chunk 会基于第 t=10 的截图推理出 16 个 action；随后每执行其中一个 action，下一轮都会拿到新的环境截图并保存，但不会重新 query 模型，直到这 16 个 action 执行完、队列为空。

dummy action 是指在正式执行动作前，先执行一些无害的动作（例如保持不动），以便让系统稳定下来并获取初始环境状态的截图。这些 dummy action 不会对环境造成实际影响，但可以确保模型在开始正式动作之前有一个清晰的环境观测。代码注释写的是 “let objects stabilize”，也就是让仿真刚 reset / set_init_state 后，物体和机器人状态在物理引擎里稳定一下。很多仿真环境 reset 后的前几帧可能有接触、重力、关节控制器初始化等瞬态，直接拿第 0 帧给 policy 可能不是训练/评估期望的稳定初始状态。 dummy action 在 LIBERO 这里定义为：return [0, 0, 0, 0, 0, 0, -1]。它的注释是 “dummy/no-op action, used to roll out the simulation while the robot does nothing”。前 6 维是 0，表示末端位姿/速度类控制量不动；最后一维是 gripper action，-1 通常表示保持或关闭方向，具体语义取决于 LIBERO/robosuite 的 gripper controller，但在这里作者把它当 no-op/等待动作使用。

时间线是：

  - t=0..9：执行 10 步 dummy action，不 query policy，不保存 rollout 图。
  
  - t=10：第一次拿当前 observation，也就是 10 步 dummy 后的状态，query policy。
  
  - policy 返回 chunk_size=16 个 action。
  
  - 接下来执行这 16 个 policy action，对应环境步大致是 t=10..25。
  
  - 执行完后 t=26，action_queue 为空，再拿当前 observation query 下一个 chunk。

  所以第一个 action chunk 覆盖的是 warmup 后的未来 16 步，不包含前 10 步 dummy action。
...
## DiT4DiT：把视频生成模型变成机器人控制的“物理先验”
现有 VLA 模型虽然在语言理解和视觉语义上很强，但它们的大部分表示能力来自静态图文预训练，真正的物理动态、接触关系、连续运动和状态转移，仍然主要依赖有限的机器人轨迹数据来学习。作者认为，这正是 VLA 在复杂操作任务中泛化不足的根源。相比之下，视频生成模型在大规模视频上学习过“世界如何随时间变化”，天然包含时空连续性、运动规律和隐式物理先验。因此，DiT4DiT 想回答一个问题：**能不能把视频生成模型不只是当作辅助模块，而是当作机器人策略学习的核心 backbone？** 论文给出的答案是肯定的：视频生成不仅可以预测未来画面，还可以作为 action policy 的可扩展训练目标和条件表示。过去很多方法要么把视频模型用于生成额外数据，要么先预测未来视频，再训练一个 inverse dynamics model 从未来状态反推动作。这些方法虽然利用了视频动态，但通常是多阶段、松耦合的：视频模型学视频，动作模型再单独学动作，中间的表示未必真正为控制服务。DiT4DiT 的关键转变是：它提出一个端到端的 Video-Action Model，用一个 Video DiT 建模未来视觉动态，再用一个 Action DiT 根据视频生成过程中的中间特征预测机器人动作，两个 DiT 通过 joint training 同时优化。

DiT4DiT 的方法核心不是“先生成最终未来帧，再把未来帧喂给动作模型”。作者认为，最终解码出来的视频虽然直观，但它可能过度关注像素级重建细节，反而不是最适合控制的表示。真正有价值的是视频扩散生成过程中的**中间去噪特征**。因此，DiT4DiT 在 Video DiT 的去噪过程中插入 hook，从特定 flow timestep $\tau_f$ 抽取 hidden states，把这些 intermediate generative features 作为 Action DiT 的条件。这样，动作模型看到的不是静态图像语义，也不是最终 RGB 视频，而是视频生成模型在“想象未来”过程中形成的时空动态表示。
整个框架可以理解成“双 DiT 级联”。第一个 DiT 是 Video DiT，初始化自 Cosmos-Predict2.5-2B，用 causal video VAE 把当前观测和未来帧压缩到 latent 空间，再通过 diffusion transformer 学习未来视觉动态。第二个 DiT 是 Action DiT，结构上借鉴 GR00T-N1 的 action diffusion transformer，用机器人 proprioceptive state、noisy action trajectory 和 learnable future tokens 作为输入，并通过 cross-attention 读取 Video DiT 抽取的中间视觉动态特征，最终输出连续机器人动作。论文图 2 展示了这个流程：当前观测和语言指令进入 Video DiT，Video DiT 预测未来动态并暴露中间 generative features，Action DiT 再根据这些 features 生成控制轨迹。

![](./Embodied_World_Models/dit4dit.png)

训练上，DiT4DiT 使用 dual flow-matching objective，把视频生成和动作生成统一起来。Video DiT 学习从噪声 latent 生成未来视频 latent；Action DiT 学习从 noisy action trajectory 去噪出真实动作。两者不是简单共享一个 diffusion timestep，而是使用不对称的 tri-timestep 设计：视频模块的 $\tau_v$ 使用 uniform sampling，以覆盖完整去噪轨迹；动作模块的 $\tau_a$ 使用 Beta sampling，使训练更关注关键控制阶段；视觉特征抽取则使用固定 timestep $\tau_f$，保证 Action DiT 每次接收到稳定、一致的中间视频特征。这一设计背后的直觉是，视频生成需要学习完整未来动态，而动作控制更需要稳定的、可执行的中间表示。

推理阶段，DiT4DiT 很灵活：它既可以单独生成未来视频，也可以单独预测动作，还可以同时做两者。**若执行动作预测，系统并不需要完整跑完多步视频解码后再输入给 action model，而是在固定 $\tau_f$ 处通过一次 video backbone 前向抽取 hidden states，然后 Action DiT 以这些 hidden states 和机器人状态为条件，从噪声动作轨迹中迭代去噪得到最终动作**。这个设计的好处是，它把视频生成模型的物理动态先验转移给动作模型，但避免把控制完全建立在最终像素视频上。

在实验设置上，论文先验证了一个基础问题：视频生成是否真的是一个比 grounding 或 VLM-style latent prediction 更好的 policy learning proxy。作者在 RoboCasa-GR1 的 24 个 tabletop manipulation tasks 上比较了三类预训练目标：object-level grounding、FLARE-style latent modeling，以及 video generation。结果显示，video generation objective 在样本效率和收敛速度上明显更好，最高带来超过 10× 的数据效率提升和最多 7× 的收敛速度提升。这一点是论文的关键论证：作者不是一上来就说 Video DiT 很强，而是先证明“学习未来视频动态”本身比静态语义对齐更适合作为机器人控制的 scaling proxy。

正式评测覆盖了仿真和真实机器人。仿真方面，DiT4DiT 在 LIBERO 上取得 98.6% 的平均成功率，其中 LIBERO-Object 为 99.6%，LIBERO-Goal 为 98.6%，LIBERO-Long 为 97.6%，整体超过多个强 VLA baseline。更重要的是，DiT4DiT 是 from scratch 训练的，也就是没有使用当前 benchmark 之外的额外 action 数据，这使得结果更能说明视频动态建模本身的贡献。在更复杂的 RoboCasa-GR1 tabletop benchmark 上，DiT4DiT 的平均成功率达到 50.8%，超过 GR00T-N1.5 的 41.8%、GR00T-N1.6 的 40.8%，也明显高于参数规模相近的 Qwen3DiT baseline 的 36.2%。这个对比尤其重要，因为 Qwen3DiT 使用的是 Qwen3-VL 作为视觉语言基础模型，再接同样的 Action DiT；DiT4DiT 则把 Qwen3-VL 替换成视频生成 backbone。二者的差距说明，对复杂 inverse dynamics 来说，静态图文语义先验不如视频生成模型中的时空物理先验有效。

真实机器人实验部署在 Unitree G1 上，机器人使用双 7-DoF 手臂和 ALOHA2 夹爪，共 16-DoF 连续动作空间，并且只依赖 egocentric camera 作为视觉输入。任务包括 pick and place、arrange the flower、stack up the cups、insert plate into the rack、box packaging、move the spoon 和 drawer interaction，每个任务采集 200 条人类示范，评测时每个任务做 20 次真实 rollout。结果显示，DiT4DiT 在七个真实任务上整体优于 GR00T-N1.5 和 Qwen3DiT；比如 Arrange Flower 达到 75%，而 GR00T-N1.5 为 25%；Stack Cup 为 60% vs. 25%；Move Spoon 为 40% vs. 15%。在 Drawer Interaction 和 Box Packaging 这类多阶段长程任务中，DiT4DiT 分别达到 90% 和 50%，说明它不仅能做短程抓取，也能支持更长 horizon 的物理执行。

论文还专门分析了泛化能力。在 RoboCasa 的 object-substitution 设置中，训练只涉及 bottle，测试时替换成 can、cup、milk、wine 等未见物体，DiT4DiT 在 ToDrawerClose、ToCabinetClose、ToMicrowaveClose 等任务上都明显超过 Qwen3DiT。真实机器人中也测试了材质、形状、目标物体和数量变化，例如把塑料杯换成金属/玻璃杯、把 eggplant 换成 corn、改变杯子数量并增加 distractors。作者认为，视频生成特征学习到的是更底层的物理交互和场景动态，因此比静态 VLM 表示更能抵抗表面外观变化。

消融实验进一步解释了为什么 DiT4DiT 有效。首先，抽取 Video DiT 哪一层 hidden states 很重要：早层主要编码低级纹理，控制效果差；中深层效果最好，默认选择 layer 18；最终层反而性能下降，因为它们过度服务于像素级视频重建，丢失了更抽象的控制相关表示。其次，动作条件并不需要多步视频去噪特征，单次 forward step 反而最好；随着 denoising steps 增加，性能单调下降，说明过度贴合具体未来像素细节会伤害可泛化 action prior。最后，joint training 比 video/action 分离训练更能形成平滑的时间阶段表示，t-SNE 中 early/middle/late phase 的分布更清晰，silhouette score 也接近翻倍。

从效率角度看，DiT4DiT 并不是完全没有代价。它有 2.2B trainable parameters，部署频率为 6Hz，低于 Qwen3DiT 的 9Hz 和 GR00T-N1.5 的 13Hz。但作者认为这是视频生成 backbone 带来的合理 trade-off，因为模型获得了更强的时空动态理解和真实世界迁移能力。更关键的是，消融实验中单步隐藏特征抽取已经证明：动作控制不需要完整多步视频生成，这为后续做更高频、异步、缓存式 Video Planner/Action Executor 提供了直接启发。
...
## [(Adaptive action chunking) Mixture of Horizons in Action Chunking](https://arxiv.org/pdf/2511.19433)
...
## [(Adaptive action chunking) Adaptive Action Chunking at Inference-time for Vision-Language-Action Models](https://arxiv.org/pdf/2604.04161)
...
## [(Adaptive action chunking) Open-Loop Planning, Closed-Loop Verification: Speculative Verification for VLA](https://arxiv.org/pdf/2604.02965)
...
## [(Adaptive action chunking) When to Trust Imagination: Adaptive Action Execution for World Action Models](https://arxiv.org/pdf/2605.06222v1)
...
### r 是什么意思？
action 的频率比 video token / visual prediction 的频率高, 论文里：$\hat{A}*t=[\hat{a}*t,\hat{a}*{t+1},...,\hat{a}*{t+k}]$, 表示从当前检查时刻 (t) 开始，剩余的 **低层动作序列**。

而：$\hat{O}*{tf}=[\hat{o}*{t+r},\hat{o}*{t+2r},...,\hat{o}*{t+k}]$ 表示 WAM 预测的未来视觉 token / latent frame。

其中：$r$ 是 **action-to-video frequency ratio**，也就是：

> 每执行 (r) 个 action step，才对应一个预测视觉帧 / visual token。

例如：$r=4$, 表示：

```text
动作：$a_t, a_{t+1}, a_{t+2}, a_{t+3}$
视觉：对应预测一帧 $\hat{o}_{t+4}$
```

所以视觉预测不是每个 action step 都有一帧，而是低频的。


### 每次 action chunk size 是 (k) 吗？

在 FFDC 检测时，$\hat{A}_t=[\hat{a}*t,...,\hat{a}*{t+k}]$ 表示**当前检查时刻剩余待验证的 action segment 长度**，可以理解为 verifier 当前看的窗口长度是 (k)。

但整个 WAM 一次生成的完整 action chunk 可能更长，比如论文实验里的 long-chunk backbone 是 **64 actions**。FFDC 在执行过程中会检查当前剩余部分是否可信，而不是固定每次都重新生成 (k) 个动作。论文说明 LC-16/32/48/64 是训练 chunk size 64 的长 chunk backbone，分别执行前 16、32、48、64 个动作；FFDC-WAM 则是自适应执行方法。([arXiv][1])

所以：$k$更像是 **FFDC 当前验证窗口 / remaining segment horizon**，不一定等同于完整 WAM chunk size。

### 一个 action chunk 下会预测多少帧未来图像状态？

如果 action chunk 长度是：$H$, action-to-video frequency ratio 是：$r$, 那么未来视觉 token 数量大约是：$\frac{H}{r}$
...
```text
16 个未来视觉 token / latent frames
```

对应：$\hat{o}*{t+4},\hat{o}*{t+8},...,\hat{o}_{t+64}$


论文中也写到未来视觉 segment 是：$\hat{O}*{tf}=[\hat{o}*{t+r},\hat{o}*{t+2r},...,\hat{o}*{t+k}]$, 其中 $r$ 就是 action-to-video frequency ratio。


### FFDC 是每个 step 检测一次，还是每个 chunk 检测一次？

更准确地说：

> **FFDC 不是每个低层 action step 都检测一次，也不是只在整个 chunk 结束后检测一次，而是在执行 chunk 过程中，按视觉 token 对齐的检查点进行检测。**

也就是大致每隔 (r) 个 action step 检测一次。
...
这些与预测视觉 token 对齐的位置进行检查。

每次 FFDC 判断：

```text
当前真实观测是否仍然支持继续执行剩余 action segment？
```

如果置信度高，就继续执行；如果低，就提前停止并 replanning。


### 检测时真实图像是每步获取，还是每个 chunk 获取一次？

真实机器人通常可以持续获取相机图像，但 **FFDC 不需要每个低层 action step 都编码和判断一次**。它需要在每个检查点获取当前真实图像：$o_t^{real}$. 然后编码成真实视觉 token：$O_t$, 再和 WAM 预测的 past/future visual tokens、剩余 actions、语言指令一起输入 FFDC：

$X_t=[L,\hat{O}*{tp},O_t,\hat{O}*{tf},\hat{A}_t,[CLS]]$


论文明确给出了这个 verifier 输入形式。([arXiv][1])
...
执行 $a_1$ 到 $a_4$
获取真实图像 $o_4^real$
FFDC 检查：真实 $o_4$ 是否符合预测 $\hat{o}_4$，以及剩余动作 $a_5...a_64$ 是否还可信
...
如果不可信：
停止当前 chunk
重新调用 WAM，从最新真实观测重新规划
```

* $\hat{A}_t=[\hat{a}*t,...,\hat{a}*{t+k}]$：当前剩余待验证的动作片段。
* $\hat{O}*{tf}=[\hat{o}*{t+r},\hat{o}*{t+2r},...,\hat{o}*{t+k}]$：与动作片段对应的低频未来视觉 token。
* $r$：每 $r$ 个 action step 对应一个 future visual token。
* 完整 action chunk size 不一定是 $k$，实验里 WAM 可一次预测 64 个 actions。
* 未来视觉帧数量约为：$H/r$
* FFDC 通常按视觉对齐点检测，而不是每个低层 action step 检测。
* 检测时需要获取当前真实图像，并编码成 (O_t)，但不是只在整个 chunk 结束后才获取。
